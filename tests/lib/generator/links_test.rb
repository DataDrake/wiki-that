...
class LinkGenTest < Test::Unit::TestCase
  ...
  def test_external_inline
    gen = WikiThat::HTMLGenerator.new('Go Here: [http://example.com]', 'wiki', 'BOB', 'sub/folder')
    gen.generate
    assert_true(gen.success?, 'Generation should have succeeded')
  ...
  def test_external_inline2
    gen = WikiThat::HTMLGenerator.new('[http://example.com] <-- Follow', 'wiki', 'BOB', 'sub/folder')
    gen.generate
    assert_true(gen.success?, 'Generation should have succeeded')
    assert_equal('<p><a href="http://example.com"></a> <-- Follow</p>', gen.result)
  end

  def test_external_alt
    gen = WikiThat::HTMLGenerator.new('[http://example.com Example]', 'wiki', 'BOB', 'sub/folder')
    gen.generate
    assert_true(gen.success?, 'Generation should have succeeded')
  ...
  def test_interwiki_named
    gen = WikiThat::HTMLGenerator.new('[[Test123:/public/Home|Home]]', 'wiki', 'BOB', 'sub/folder')
    gen.generate
    assert_true(gen.success?, 'Generation should have succeeded')
